...
import (
	"context"
	"golang-crud/constants"
	"golang-crud/graph/model"
	"log"
	"time"
...
func (db *DB) GetBookById(id string) *model.BookListing {
	bookCollection := db.client.Database(constants.DatabaseInfo[constants.DatabaseName]).Collection(constants.DatabaseInfo[constants.CollectionName])
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
